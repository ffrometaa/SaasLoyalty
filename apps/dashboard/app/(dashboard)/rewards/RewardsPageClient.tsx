...
import { Plus, Gift, Edit, X, Eye, EyeOff } from 'lucide-react';
...
export function RewardsPageClient({ initialRewards }: Props): JSX.Element {
  ...
  async function handleCreate(formData: RewardFormData): Promise<void> {
    setLoading(true);
    setError(null);
    try {
      const res = await fetch('/api/rewards', {
  ...
  async function handleUpdate(id: string, updates: Partial<RewardFormData>): Promise<void> {
    setLoading(true);
    setError(null);
    try {
    ...
    <div className="p-6 lg:p-8">
      ...
      <div className="mb-6 flex items-center gap-2">
        <button
          ...
          className={clsx('px-4 py-2 rounded-lg text-sm font-medium transition-colors', filter === 'all' ? 'bg-brand-purple-100 text-brand-purple-700' : 'bg-gray-100 text-gray-600 hover:bg-gray-200')}
        >
          {t('all', { count: rewards.length })}
        </button>
        <button
          onClick={() => setFilter('active')}
          className={clsx('px-4 py-2 rounded-lg text-sm font-medium transition-colors', filter === 'active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600 hover:bg-gray-200')}
        >
          {t('active', { count: activeCount })}
        </button>
        <button
          onClick={() => setFilter('inactive')}
          className={clsx('px-4 py-2 rounded-lg text-sm font-medium transition-colors', filter === 'inactive' ? 'bg-gray-200 text-gray-700' : 'bg-gray-100 text-gray-600 hover:bg-gray-200')}
        >
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {filteredRewards.map((reward) => (
          <div
            key={reward.id}
            className={clsx('bg-white rounded-xl border overflow-hidden transition-all', !reward.is_active ? 'opacity-60 bg-gray-50' : 'hover:shadow-lg')}
          >
            <div className="p-6">
              <div className="flex items-start justify-between">
                <div className={clsx('p-3 rounded-lg', reward.is_active ? 'bg-brand-purple-100' : 'bg-gray-100')}>
                  <Gift className={clsx('h-6 w-6', reward.is_active ? 'text-brand-purple' : 'text-gray-400')} />
                </div>
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => handleToggleActive(reward.id)}
                    className={clsx('p-1.5 rounded-lg transition-colors', reward.is_active ? 'text-green-600 hover:bg-green-50' : 'text-gray-400 hover:bg-gray-100')}
...
function RewardModal({
  ...
  const [formData, setFormData] = useState<{
    ...
    points_required: reward ? String(reward.points_required) : '',
    max_redemptions: '',
    valid_until: '',
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={clsx('relative inline-flex h-6 w-11 items-center rounded-full transition-colors', formData.is_active ? 'bg-brand-purple' : 'bg-gray-200')}
            >
              <span
                className={clsx('inline-block h-4 w-4 transform rounded-full bg-white transition-transform', formData.is_active ? 'translate-x-6' : 'translate-x-1')}
              />
            </button>
