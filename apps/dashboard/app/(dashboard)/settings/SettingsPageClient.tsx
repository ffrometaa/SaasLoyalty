...
export interface SettingsData {
  ...
  pointsPerVisitEnabled?: boolean;
  pointsPerVisit?: number;
  welcomeBonusEnabled?: boolean;
  welcomeBonusPoints?: number;
  googleReviewUrl?: string;
  googleReviewBonusEnabled?: boolean;
  googleReviewBonusPoints?: number;
...
export function SettingsPageClient({ initialSettings }: Props): JSX.Element {
  ...
  type TabId = 'profile' | 'branding' | 'member-app' | 'billing' | 'language' | 'team' | 'danger' | 'integrations' | 'loyalty' | 'feedback';
  ...
  useEffect(() => {
    ...
    if (data) {
      ...
      // No settings provided by server — show error state
      setSettingsLoading(false);
    }
  }, []);
  ...
  const loadTeam = async (): Promise<void> => {
    ...
    try {
      ...
      if (membersRes.ok) setTeamMembers(((await membersRes.json()) as { members: TeamMember[] }).members ?? []);
      if (invitesRes.ok) setTeamInvites(((await invitesRes.json()) as { invites: TeamInvite[] }).invites ?? []);
    } finally {
  ...
  const loadIntegrations = async (): Promise<void> => {
    ...
    try {
      ...
      if (res.ok) setIntegrations(await res.json() as { apiKey: string; slug: string; joinUrl: string; widgetUrl: string });
    } finally {
  ...
  const handleSendInvite = async (e: React.FormEvent): Promise<void> => {
    e.preventDefault();
  ...
  const handleRevokeInvite = async (id: string): Promise<void> => {
    const res = await fetch(`/api/team/invites/${id}`, { method: 'DELETE' });
    if (!res.ok) console.error('[handleRevokeInvite] failed:', await res.json().catch(() => null));
    loadTeam();
  };
  ...
  const handleRemoveMember = async (id: string): Promise<void> => {
    ...
    if (!res.ok) console.error('[handleRemoveMember] failed:', await res.json().catch(() => null));
    loadTeam();
  };
  ...
  const loadUsage = async (): Promise<void> => {
    try {
      ...
      if (res.ok) setUsage(await res.json() as { activeMembers: number; memberLimit: number | null; pointsIssuedThisMonth: number });
    } catch (err) {
  ...
  const handleSameAsBusiness = (checked: boolean): void => {
  ...
  const handleSaveLoyalty = async (): Promise<void> => {
  ...
  const PLAN_INFO: Record<string, { price: string; memberLimit: string; features: string[] }> = {
    starter: { price: '$99', memberLimit: '800', features: ['Up to 800 members', 'Basic analytics', '3 campaigns/month', 'Email support'] },
    pro: { price: '$399', memberLimit: '2,000', features: ['Up to 2,000 members', 'Advanced analytics', '10 campaigns/month', 'Priority support'] },
            ...
            {tabs.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={clsx('w-full flex items-center gap-3 px-4 py-3 text-sm font-medium rounded-lg transition-colors', activeTab === tab.id ? 'bg-brand-purple-50 text-brand-purple' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900')}
                >
                      ...
                      <tbody className="divide-y divide-gray-100">
                        {invoices.map((inv) => (
                          <tr key={inv.id}>
                            ...
                            <td className="py-3">
                              <span className={clsx('inline-flex px-2 py-0.5 text-xs font-medium rounded-full capitalize', inv.status === 'paid' ? 'bg-green-100 text-green-800' : inv.status === 'open' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-800')}>
                                {inv.status}
                              </span>
                            </td>
                ...
                {[
                  ...
                  return (
                    <button
                      ...
                      className={clsx('flex items-center gap-3 p-4 rounded-xl border-2 transition-colors text-left', isActive ? 'border-brand-purple bg-brand-purple-50' : 'border-gray-200 hover:border-gray-300')}
                    >
                      <span className="text-2xl">{flag}</span>
                      <div>
                        <p className={clsx('font-medium', isActive ? 'text-brand-purple' : 'text-gray-900')}>{label}</p>
            ...
            <div className="bg-white rounded-xl border p-6 space-y-8">
              ...
              <div className="pt-6 border-t">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-xl">
                  {[
                    { label: 'Silver', textClass: 'text-slate-400', key: 'silverThreshold' as const },
                    { label: 'Gold', textClass: 'text-amber-500', key: 'goldThreshold' as const },
                    { label: 'Platinum', textClass: 'text-violet-500', key: 'platinumThreshold' as const },
                  ].map(({ label, textClass, key }) => (
                    <div key={key}>
                      <label className={clsx('block text-sm font-medium mb-1', textClass)}>
                        {label}
                      </label>
                      <input
                        type="number"
              ...
              <div className="pt-6 border-t">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button
                    ...
                    className={clsx('relative inline-flex h-6 w-11 items-center rounded-full transition-colors', loyaltyRules.pointsPerVisitEnabled ? 'bg-brand-purple' : 'bg-gray-200')}
                  >
                    <span className={clsx('inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform', loyaltyRules.pointsPerVisitEnabled ? 'translate-x-6' : 'translate-x-1')} />
                  </button>
                </div>
              ...
              <div className="pt-6 border-t">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button
                    ...
                    className={clsx('relative inline-flex h-6 w-11 items-center rounded-full transition-colors', loyaltyRules.welcomeBonusEnabled ? 'bg-brand-purple' : 'bg-gray-200')}
                  >
                    <span className={clsx('inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform', loyaltyRules.welcomeBonusEnabled ? 'translate-x-6' : 'translate-x-1')} />
                  </button>
                </div>
              ...
              <div className="pt-6 border-t">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button
                    ...
                    className={clsx('relative inline-flex h-6 w-11 items-center rounded-full transition-colors', loyaltyRules.googleReviewBonusEnabled ? 'bg-brand-purple' : 'bg-gray-200')}
                  >
                    <span className={clsx('inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform', loyaltyRules.googleReviewBonusEnabled ? 'translate-x-6' : 'translate-x-1')} />
                  </button>
                </div>
              ...
              <div className="pt-6 border-t">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button
                    ...
                    className={clsx('relative inline-flex h-6 w-11 items-center rounded-full transition-colors', loyaltyRules.referralEnabled ? 'bg-brand-purple' : 'bg-gray-200')}
                  >
                    <span className={clsx('inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform', loyaltyRules.referralEnabled ? 'translate-x-6' : 'translate-x-1')} />
                  </button>
                </div>
