...
export default function InvitePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  useEffect(() => {
    ...
    // Load invite info + session in parallel
    Promise.all([
      fetch(`/api/invite/${token}`).then(r => r.json()),
      (supabase.auth as any).getSession(),
    ]).then(([inviteData, { data: { session: sess } }]) => {
      if (inviteData.error) {
        setInviteError(inviteData.error);
      } else {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data: { session: newSession } } = await (supabase.auth as any).getSession();
    ...
    return (
      <div className="auth-container">
        <div className="auth-card text-center">
          <div className="h-8 w-8 animate-spin rounded-full border-4 border-[#7c3aed] border-t-transparent mx-auto" />
        </div>
      </div>
    );
  }
    ...
    <div className="auth-container">
      <div className="auth-card">
        <div className="text-center mb-6">
          <div className="h-14 w-14 rounded-2xl bg-[#7c3aed]/20 flex items-center justify-center mx-auto mb-4">
